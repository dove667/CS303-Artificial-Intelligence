...
class ProblemSolvingAgent:
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        ...
        while frontier:
            ...
            for neighbor, cost in self.neighbours_of(obstacles, state):
    ...
    def UCS(self, obstacles, start_pos, goal_pos):
        ...
        while frontier:
            ...
            for neighbor, weight in self.neighbours_of(obstacles, current_node):
                if neighbor in explored:
                    continue
                new_cost = cost + weight
                if neighbor not in node_info or new_cost < node_info[neighbor]:
                    node_info[neighbor] = new_cost
    ...
    def GBFS(self, obstacles, start_pos, goal_pos):
        ...
        while frontier:
            heuristic, current_node = heapq.heappop(frontier)
            ...
            for neighbor, _ in self.neighbours_of(obstacles, current_node):
                if neighbor in explored:
                    continue
                new_heuristic = self.euclidean_distance(goal_pos, neighbor)
                if neighbor not in node_info or new_heuristic < node_info[neighbor]:
                    node_info[neighbor] = new_heuristic
                    parents[neighbor] = current_node
                    heapq.heappush(frontier, (new_heuristic, neighbor))
        return path, visited
    
    def Astar(self, obstacles, start_pos, goal_pos):
        path, visited = [], []
        frontier = [] # (estimated_cost, 节点)
        node_info = {} # {节点: estimated_cost}
        explored = set()
        parents = {start_pos: None}
        heapq.heappush(frontier, (0, start_pos))
        node_info[start_pos] = 0
        ...
        while frontier:
            ...
            for neighbor, moving_cost in self.neighbours_of(obstacles, current_node):
                if neighbor in explored:
                    continue
                new_path_cost = estimated_cost - self.euclidean_distance(goal_pos, current_node) + moving_cost
                new_estimated_cost = new_path_cost + self.euclidean_distance(goal_pos, neighbor)
                if neighbor not in node_info or new_estimated_cost < node_info[neighbor]:
                    node_info[neighbor] = new_estimated_cost
                    parents[neighbor] = current_node
                    heapq.heappush(frontier, (new_estimated_cost, neighbor))
    ...
    def euclidean_distance(self, node1, node2, coefficient=1):
        ...
        """
        return coefficient * sqrt((node1[0]-node2[0])**2 + (node1[1]-node2[1])**2)
